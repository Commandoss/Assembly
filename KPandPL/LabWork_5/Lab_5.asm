...
reading: 
    ...
    call read_file           ;ax <- eie-ai n?eoaiiuo aaeo 
    ...
    ;caiinei a si e di oeacaoaee ia ia?aei no?ie
    mov si, offset file_buffer
    mov di, offset word_
                          
    sub al, dl  ;iaoiaei iaiaoiaeiue ?acia? eoa?aoee aey i?ioi?aaiey oeeea loop1
    
    xor cx, cx      
    mov cl, al  ;caiinei a cx eie-ai eoa?aoee aey oeeea loop1      
    inc cx
    ...
    word_finding_loop:
        ...
        ;n?aaieaaai no?iee     
        xor cx, cx 
        mov cx, dx          ;caiinei a cx eie-ai eoa?aoee
        repe cmpsb          ;iiaoi?yai iiea ?aaiu
        jnz continue_       ;anee ?aaiu goto find
        ...
        popa                ;auiinei ec noaea io?iua ?aaeno?u   
        
        mov start_buffer_flag, 0
                         
        inc si                      ;ia?aoiaei ia neaao?uee neiaie a iiaie no?iea                                      
        mov di, offset word_        ;na?anuaaai n?ao?ee                                  
   ...
   ;si - iinea auiieiaiey oeeea oeacuaaao ia eiiao iiano?iee a no?iea
   find:
       ...
       ;call outputString                                                      
       ;caiinei a di aa?an ia?aea ia?aie iiano?iee                              
       mov di, offset file_buffer   
       ;caiinea a cx aa?an eiioa iiano?iee a no?iea
       mov cx, si              
       ;iaoiaei ?acia? ia?aie ?anoe no?iee n iiano?ieie (io ia?aea ia?aie no?iee ai eiioa aoi?ie iiano?iee a ia?aie)
       sub cx, offset file_buffer  
       
       xor ax, ax
       mov ax, size_of_word
              
       ;caiinei a al ?acia? aoi?ie iiano?iee            
       ;mov ax, size_of_string
       ;iaoiaei ?acia? ia?aie ?anoe no?iee aac iiano?iee 
       sub cx, ax 
