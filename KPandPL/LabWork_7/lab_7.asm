newline MACRO          ; Переходит на след строку     
	push ax                           
	push dx                           
...
println MACRO info     ; Печатает строку и переходит на след строку    
	push ax                          
	push dx                           
...
printErrorCode MACRO                     
	add al, '0'         ; Выводит код ошибки (ax - error code)
	mov dl, al                        
	mov ah, 06h                       
	int 21h                           
...
startFolder         db maxCMDSize + 2 dup(0)
startDisk db "c:\", 0x00
...
EPBstruct dw 0  	                 ; текущее окружение 
	  dw offset line, 0              ; Адрес командной строки  
	  dw 005Ch, 0, 006Ch, 0    	     ; Адреса FCB (File control block) программы
line                db 125           ; Длина командной строки     
...
main:                                     
	; reallocating memory             ; Сокращаем выделенную программе память до минимума(ES-пар. откуда нач. своб. память
	mov ah, 4Ah                       
	mov bx, ((CodeSize/16)+1)+((DataSize/16)+1)+2*(256/16) ; требуемый размер в параграфах (учитываем отброшенный остаток)
	int 21h
...
initToRunAllGood:                         
	...
	mov ah, 47h	
    mov dl, 00h
    mov si, offset startFolder                                 ; теперь в cmd_text то, что было записано в командную строку
	int 21h   
	pop dx
	pop si
	pop ds  
...
endMain:             ; Прерывание для выполнения функции                       ;exit  
    newline                              
	println endText 
	                   ; Выводим заключительный текст   
	push ds
	push ax
	xor ax, ax
    mov ah, 3Bh
    mov dx, offset startDisk
    int 21h 
    pop ax        
    pop ds 
...
; если в ax 0 то все окей ну если нет то нет)                                        
parseCMD PROC                             
	push bx			
	push cx
	...
	call rewriteAsASCIIZWord          
	cmpWordLenWith0 folderPath, badCMDArgs ; Если название нулевой длины - ошибка
...
;ds: si - смещение, где найдем (остановка результата тоже будет в si)
;es: di - смещение, где будет слово
;cx - максимальный размер слова (ввод)
; результат будет ASCIIZ
...
installDTA PROC                        ; Указывает область памяти, которая будет использоваться для последующих операций FCB.   
...
CodeSize = $ - main                      ; Размер сегмента кода 
                                          
end main                  

;Users/ilya/Projects/MyProject/MacNoEditor/Solarsystem.app                